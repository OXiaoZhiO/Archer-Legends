...
## 项目简介
学校的编程比赛作品，自己做的一个pygame小游戏，编程环境是pycharm，用了一点ai

**弓箭手传奇** 是一款基于 Pygame 开发的 2D 射箭小游戏。玩家通过控制弓箭手发射箭矢击中移动的靶子来获得分数。游戏包含动态生成的靶子、蓄力系统、物理模拟（如重力影响），以及一个可滚动的背景。
### 核心功能
...
project/
├── objects/            # 游戏对象相关类
│   ├── arrow.py        # 箭矢类，处理箭的运动、绘制和碰撞检测
│   ├── target.py       # 靶子类，处理靶子的生成、移动和碰撞检测
│   └── power_bar.py    # 蓄力条类，用于显示和管理蓄力状态
├── settings.py         # 游戏配置文件，包括颜色、屏幕尺寸等常量
├── utils/              # 工具函数和辅助模块
│   ├── transform.py    # 坐标转换工具
│   ├── drawing.py      # 绘图工具函数
│   ├── debug.py        # 调试工具函数
│   └── start_menu.py   # 启动菜单逻辑
├── shoot.py            # 游戏主程序入口
└── README.md           # 项目说明文档
```
## 如何运行
...
## 功能详解
### 1. 靶子生成与移动
靶子由 类管理，随机从屏幕左侧或右侧生成，并向相反方向移动。当靶子超出屏幕边界时会反弹。 `Target`
### 2. 箭矢发射与物理模拟
箭矢由 类管理，支持以下特性： `Arrow`
- **蓄力系统**: 按住鼠标左键蓄力，松开后发射箭矢。
- **重力模拟**: 箭矢在飞行过程中受到重力影响，轨迹呈抛物线。
- **边界检测**: 箭矢飞出屏幕后会被移除。
...
- GitHub: https://github.com/OoXiaoZhioO
